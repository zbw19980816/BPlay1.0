    ...
    while (ffmpegrun) {
        ...
        if (pkt.stream_index == Audio_index) {
           ...
           // BLOG("音频流 %d", c);
            ...
            BLOG("音频流 %d  pts %d  timebase[%d]", c, pkt.pts, FormatContext->streams[Audio_index].time_base);
            QMutexLocker locker(&AudioQue.mtx);
            AudioQue.que.push_back(pkt);
        } else if (pkt.stream_index == Video_index) {
            /* 视频流入队列 */
            //BLOG("视频流 %d", c);
            BLOG("视频流 %d  pts %d  timebase[%d]", c, pkt.pts, FormatContext->streams[Video_index].time_base);
